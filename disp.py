...
# FOR doped-RNN project
def quiver(ax, flow, RX, c='k', c_map='PuOr', c_scale=1):  # plot flow field
    if flow.shape[2] == 3:
        dx = np.mean(np.diff(RX))
        extent = 2*[RX[0]-dx/2, RX[-1]+dx/2]

        flow_2_abs_max = np.nanmax(np.abs(flow[:, :, 2]))
        if flow_2_abs_max < 1e-5:
            v_min = -1
            v_max = 1
        else:
            v_min = -flow_2_abs_max*c_scale
            v_max = flow_2_abs_max*c_scale
            
        ax.matshow(flow[:, :, 2].T, origin='lower', extent=extent, cmap=c_map, vmin=v_min, vmax=v_max)
        
    ax.quiver(RX, RX, flow[:, :, 0].T, flow[:, :, 1].T, lw=2, color=c)
    return ax